<script>
    ...
    const deleteFoodInventory = () => {
      try {
        inventory.update((curr) => {
          curr.foods = [];
          return curr;
        });
        alert("Az élelmiszer készlet sikeresen törölve!")
      } catch (error) {
        console.error("Hiba történt az élelmiszer készlet törlésekor", error);
        alert("Nem sikerült törölni az élelmiszer készletet.");
      }
    };
    ...
    const addItem = () => {
      ...
      try {
        inventory.update((curr) => {
          ...
          curr.foods.push({ item, quantity, unit, store });
          return curr;
        });
        item = '';
        quantity = 0;
    ...
    const deleteItem = (index) => {
      inventory.update((curr) => {
        ...
        updatedInventory.foods = curr.foods.filter((_, i) => i !== index);
        return updatedInventory;
      });
    };
  </script>
  
  <h1>Élelmiszer készlet</h1>
  ...
  <table class="table table-bordered">
    ...
    <tbody>
      {#each $inventory.foods || [] as foods, index}
        <tr>
          <td>{foods.item}</td>
  ...
  <footer class="bg-light border-top py-3 fixed-bottom">
    <div class="container text-center">
      ...
      <button class="btn btn-danger btn-sm" on:click={deleteFoodInventory}>
        Élelmiszer készlet törlése
      </button>
    </div>
  </footer>
